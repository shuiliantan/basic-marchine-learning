...
def training(x_train, y_train, n_estimators: int = 10, **kwargs):
    """
...
def serving(x_predict, model, **kwargs):
    """
    ...
    y_predict['probability'] = model.predict_proba(x_predict)[:, 1]
    y_predict['prediction'] = model.predict(x_predict)
    return y_predict
...
model = training(x_train, y_train, n_estimators=7)
y_train_predict = serving(x_train, model)
# 评估

y_test_predict = serving(x_test, model)
...
print(evaluate_train_dict, evaluate_train_df )
